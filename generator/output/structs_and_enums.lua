...
defs["enum_comments"]["ImGuiCond_"]["above"] = "// Enumeration for ImGui::SetWindow***(), SetNextWindow***(), SetNextItem***() functions\
...
defs["enum_comments"]["ImGuiItemFlags_"]["above"] = "// Transient per-window flags, reset at the beginning of the frame. For child window, inherited from parent on first Begin().\
// This is going to be exposed in imgui.h when stabilized enough."
defs["enum_comments"]["ImGuiItemStatusFlags_"] = {}
defs["enum_comments"]["ImGuiItemStatusFlags_"]["above"] = "// Storage for LastItem data"
defs["enum_comments"]["ImGuiKey_"] = {}
defs["enum_comments"]["ImGuiKey_"]["above"] = "// Keys value 0 to 511 are left unused as legacy native/opaque key values (< 1.87)\
...
defs["enum_comments"]["ImGuiModFlags_"]["above"] = "// Helper \"flags\" version of key-mods to store and compare multiple key-mods easily. Sometimes used for storage (e.g. io.KeyMods) but otherwise not much used in public API."
...
defs["enum_comments"]["ImGuiNavInput_"]["above"] = "// Gamepad/Keyboard navigation\
...
defs["enum_comments"]["ImGuiNavReadMode"]["above"] = "// FIXME-NAV: Clarify/expose various repeat delay/rate"
defs["enum_comments"]["ImGuiOldColumnFlags_"] = {}
defs["enum_comments"]["ImGuiOldColumnFlags_"]["above"] = "// Flags for internal's BeginColumns(). Prefix using BeginTable() nowadays!"
...
defs["struct_comments"]["ImColor"]["above"] = "// Helpers macros to generate 32-bit encoded colors\
...
defs["struct_comments"]["ImGuiInputTextState"]["above"] = "// Internal state of the currently focused/edited text input box\
...
defs["struct_comments"]["ImGuiOnceUponAFrame"]["above"] = "// Helper: Execute a block of code at maximum once a frame. Convenient if you want to quickly create an UI within deep-nested code that runs multiple times every frame.\
...
defs["struct_comments"]["ImGuiStackTool"]["above"] = "// State for Stack tool queries"
...
defs["struct_comments"]["ImGuiTable"]["above"] = "// FIXME-TABLE: more transient data could be stored in a per-stacked table structure: DrawSplitter, SortSpecs, incoming RowData"
...
defs["struct_comments"]["ImGuiTableColumn"]["above"] = "// [Internal] sizeof() ~ 104\
...
defs["struct_comments"]["ImGuiTableInstanceData"]["above"] = "// Per-instance data that needs preserving across frames (seemingly most others do not need to be preserved aside from debug needs, does that needs they could be moved to ImGuiTableTempData ?)"
...
defs["structs"]["ImDrawData"][5]["comment"]["sameline"] = " // Array of ImDrawList* to render. The ImDrawList are owned by ImGuiContext and only pointed to from here."
...
defs["structs"]["ImDrawDataBuilder"][1]["comment"]["sameline"] = " // Global layers for: regular, tooltip"
...
defs["structs"]["ImDrawListSharedData"][8]["comment"]["above"] = "    // [Internal] Lookup tables"
defs["structs"]["ImDrawListSharedData"][8]["comment"]["sameline"] = " // Sample points on the quarter of the circle."
...
defs["structs"]["ImDrawListSharedData"][9]["comment"]["sameline"] = " // Cutoff radius after which arc drawing will fallback to slower PathArcTo()"
...
defs["structs"]["ImDrawListSharedData"][10]["comment"]["sameline"] = " // Precomputed segment count for given radius before we calculate it dynamically (to avoid calculation overhead)"
...
defs["structs"]["ImDrawListSharedData"][11]["comment"]["sameline"] = " // UV of anti-aliased lines in the atlas"
...
defs["structs"]["ImFont"][12]["comment"]["sameline"] = " // 2     // out // = '.'      // Character used for ellipsis rendering (if a single '...' character isn't found)"
...
defs["structs"]["ImFont"][13]["comment"]["sameline"] = " // 1     // out //"
...
defs["structs"]["ImFont"][14]["comment"]["sameline"] = " // 4     // in  // = 1.f      // Base font scale, multiplied by the per-window font scale which you can adjust with SetWindowFontScale()"
defs["structs"]["ImFont"][14]["name"] = "Scale"
defs["structs"]["ImFont"][14]["type"] = "float"
defs["structs"]["ImFont"][15] = {}
defs["structs"]["ImFont"][15]["comment"] = {}
defs["structs"]["ImFont"][15]["comment"]["sameline"] = " // 4+4   // out //            // Ascent: distance from top to bottom of e.g. 'A' [0..FontSize]"
...
defs["structs"]["ImFont"][17]["comment"]["sameline"] = "// 4     // out //            // Total surface in pixels to get an idea of the font rasterization/texture cost (not exact, we approximate the cost of padding between glyphs)"
...
defs["structs"]["ImFont"][18]["comment"]["sameline"] = " // 2 bytes if ImWchar=ImWchar16, 34 bytes if ImWchar==ImWchar32. Store 1-bit for each block of 4K codepoints that has one active glyph. This is mainly used to facilitate iterations across all used codepoints."
...
defs["structs"]["ImFontAtlas"][6]["comment"]["above"] = "    // [Internal]\
    // NB: Access texture data via GetTexData*() calls! Which will setup a default font for you."
defs["structs"]["ImFontAtlas"][6]["comment"]["sameline"] = " // Set when texture was built matching current font input"
...
defs["structs"]["ImFontAtlas"][7]["comment"]["sameline"] = " // Tell whether our texture data is known to use colors (rather than just alpha channel), in order to help backend select a format."
...
defs["structs"]["ImFontAtlas"][8]["comment"]["sameline"] = " // 1 component per pixel, each component is unsigned 8-bit. Total size = TexWidth * TexHeight"
...
defs["structs"]["ImFontAtlas"][9]["comment"]["sameline"] = " // 4 component per pixel, each component is unsigned 8-bit. Total size = TexWidth * TexHeight * 4"
...
defs["structs"]["ImFontAtlas"][10]["comment"]["sameline"] = " // Texture width calculated during Build()."
...
defs["structs"]["ImFontAtlas"][11]["comment"]["sameline"] = " // Texture height calculated during Build()."
...
defs["structs"]["ImFontAtlas"][12]["comment"]["sameline"] = " // = (1.0f/TexWidth, 1.0f/TexHeight)"
...
defs["structs"]["ImFontAtlas"][13]["comment"]["sameline"] = " // Texture coordinates to a white pixel"
...
defs["structs"]["ImFontAtlas"][14]["comment"]["sameline"] = " // Hold all the fonts returned by AddFont*. Fonts[0] is the default font upon calling ImGui::NewFrame(), use ImGui::PushFont()/PopFont() to change the current font."
...
defs["structs"]["ImFontAtlas"][15]["comment"]["sameline"] = " // Rectangles for packing custom texture data into the atlas."
...
defs["structs"]["ImFontAtlas"][16]["comment"]["sameline"] = " // Configuration data"
...
defs["structs"]["ImFontAtlas"][17]["comment"]["sameline"] = " // UVs for baked anti-aliased lines"
...
defs["structs"]["ImFontAtlas"][18]["comment"]["above"] = "    // [Internal] Font builder"
defs["structs"]["ImFontAtlas"][18]["comment"]["sameline"] = " // Opaque interface to a font builder (default to stb_truetype, can be changed to use FreeType by defining IMGUI_ENABLE_FREETYPE)."
...
defs["structs"]["ImFontAtlas"][19]["comment"]["sameline"] = " // Shared flags (for all fonts) for custom font builder. THIS IS BUILD IMPLEMENTATION DEPENDENT. Per-font override is also available in ImFontConfig."
...
defs["structs"]["ImFontAtlas"][20]["comment"]["above"] = "    // [Internal] Packing data"
defs["structs"]["ImFontAtlas"][20]["comment"]["sameline"] = " // Custom texture rectangle ID for white pixel and mouse cursors"
...
defs["structs"]["ImFontAtlas"][21]["comment"]["sameline"] = " // Custom texture rectangle ID for baked anti-aliased lines"
...
defs["structs"]["ImFontConfig"][6]["comment"]["sameline"] = " // 3        // Rasterize at higher quality for sub-pixel positioning. Note the difference between 2 and 3 is minimal so you can reduce this to 2 to save memory. Read https://github.com/nothings/stb/blob/master/tests/oversample/README.md for details."
...
defs["structs"]["ImFontConfig"][11]["comment"]["sameline"] = " // NULL     // Pointer to a user-provided list of Unicode range (2 value per range, values are inclusive, zero-terminated list). THE ARRAY DATA NEEDS TO PERSIST AS LONG AS THE FONT IS ALIVE."
...
defs["structs"]["ImFontConfig"][16]["comment"]["sameline"] = " // 1.0f     // Brighten (>1.0f) or darken (<1.0f) font output. Brightening small fonts may be a good workaround to make them more readable."
...
defs["structs"]["ImFontConfig"][17]["comment"]["sameline"] = " // -1       // Explicitly specify unicode codepoint of ellipsis character. When fonts are being merged first specified ellipsis will be used."
...
defs["structs"]["ImFontConfig"][18]["comment"]["above"] = "    // [Internal]"
defs["structs"]["ImFontConfig"][18]["comment"]["sameline"] = " // Name (strictly to ease debugging)"
...
defs["structs"]["ImGuiContext"][5]["comment"]["sameline"] = " // Input events which will be tricked/written into IO structure."
...
defs["structs"]["ImGuiContext"][6]["comment"]["sameline"] = " // Past input events processed in NewFrame(). This is to allow domain-specific application to access e.g mouse/pen trail."
...
defs["structs"]["ImGuiContext"][8]["comment"]["sameline"] = " // = g.IO.ConfigFlags at the time of NewFrame()"
...
defs["structs"]["ImGuiContext"][10]["comment"]["sameline"] = " // (Shortcut) == FontStack.empty() ? IO.Font : FontStack.back()"
...
defs["structs"]["ImGuiContext"][11]["comment"]["sameline"] = " // (Shortcut) == FontBaseSize * g.CurrentWindow->FontWindowScale == window->FontSize(). Text height for current window."
...
defs["structs"]["ImGuiContext"][12]["comment"]["sameline"] = " // (Shortcut) == IO.FontGlobalScale * Font->Scale * Font->FontSize. Base text height."
...
defs["structs"]["ImGuiContext"][19]["comment"]["sameline"] = " // Set by NewFrame(), cleared by EndFrame()"
...
defs["structs"]["ImGuiContext"][20]["comment"]["sameline"] = " // Set by NewFrame(), cleared by EndFrame() when the implicit debug window has been pushed"
...
defs["structs"]["ImGuiContext"][21]["comment"]["sameline"] = " // Set within EndChild()"
...
defs["structs"]["ImGuiContext"][22]["comment"]["sameline"] = " // Request full GC"
...
defs["structs"]["ImGuiContext"][23]["comment"]["sameline"] = " // Will call test engine hooks: ImGuiTestEngineHook_ItemAdd(), ImGuiTestEngineHook_ItemInfo(), ImGuiTestEngineHook_Log()"
...
defs["structs"]["ImGuiContext"][24]["comment"]["sameline"] = " // Test engine user data"
...
defs["structs"]["ImGuiContext"][25]["comment"]["above"] = "    // Windows state"
defs["structs"]["ImGuiContext"][25]["comment"]["sameline"] = " // Windows, sorted in display order, back to front"
...
defs["structs"]["ImGuiContext"][26]["comment"]["sameline"] = " // Root windows, sorted in focus order, back to front."
...
defs["structs"]["ImGuiContext"][27]["comment"]["sameline"] = " // Temporary buffer used in EndFrame() to reorder windows so parents are kept before their child"
...
defs["structs"]["ImGuiContext"][29]["comment"]["sameline"] = " // Map window's ImGuiID to ImGuiWindow*"
...
defs["structs"]["ImGuiContext"][30]["comment"]["sameline"] = " // Number of unique windows submitted by frame"
...
defs["structs"]["ImGuiContext"][31]["comment"]["sameline"] = " // Padding around resizable windows for which hovering on counts as hovering the window == ImMax(style.TouchExtraPadding, WINDOWS_HOVER_PADDING)"
...
defs["structs"]["ImGuiContext"][32]["comment"]["sameline"] = " // Window being drawn into"
...
defs["structs"]["ImGuiContext"][33]["comment"]["sameline"] = " // Window the mouse is hovering. Will typically catch mouse inputs."
...
defs["structs"]["ImGuiContext"][34]["comment"]["sameline"] = " // Hovered window ignoring MovingWindow. Only set if MovingWindow is set."
...
defs["structs"]["ImGuiContext"][35]["comment"]["sameline"] = " // [Debug] Hovered dock node."
...
defs["structs"]["ImGuiContext"][36]["comment"]["sameline"] = " // Track the window we clicked on (in order to preserve focus). The actual window that is moved is generally MovingWindow->RootWindowDockTree."
...
defs["structs"]["ImGuiContext"][37]["comment"]["sameline"] = " // Track the window we started mouse-wheeling on. Until a timer elapse or mouse has moved, generally keep scrolling the same window even if during the course of scrolling the mouse ends up hovering a child window."
...
defs["structs"]["ImGuiContext"][40]["comment"]["above"] = "    // Item/widgets state and tracking information"
defs["structs"]["ImGuiContext"][40]["comment"]["sameline"] = " // Will call core hooks: DebugHookIdInfo() from GetID functions, used by Stack Tool [next HoveredId/ActiveId to not pull in an extra cache-line]"
...
defs["structs"]["ImGuiContext"][41]["comment"]["sameline"] = " // Hovered widget, filled during the frame"
...
defs["structs"]["ImGuiContext"][44]["comment"]["sameline"] = " // Hovered widget will use mouse wheel. Blocks scrolling the underlying window."
...
defs["structs"]["ImGuiContext"][46]["comment"]["sameline"] = " // At least one widget passed the rect test, but has been discarded by disabled flag or popup inhibit. May be true even if HoveredId == 0."
...
defs["structs"]["ImGuiContext"][47]["comment"]["sameline"] = " // Measure contiguous hovering time"
...
defs["structs"]["ImGuiContext"][48]["comment"]["sameline"] = " // Measure contiguous hovering time where the item has not been active"
...
defs["structs"]["ImGuiContext"][49]["comment"]["sameline"] = " // Active widget"
...
defs["structs"]["ImGuiContext"][50]["comment"]["sameline"] = " // Active widget has been seen this frame (we can't use a bool as the ActiveId may change within the frame)"
...
defs["structs"]["ImGuiContext"][52]["comment"]["sameline"] = " // Set at the time of activation for one frame"
...
defs["structs"]["ImGuiContext"][53]["comment"]["sameline"] = " // Active widget allows another widget to steal active id (generally for overlapping widgets, but not always)"
...
defs["structs"]["ImGuiContext"][54]["comment"]["sameline"] = " // Disable losing active id if the active id window gets unfocused."
...
defs["structs"]["ImGuiContext"][55]["comment"]["sameline"] = " // Track whether the active id led to a press (this is to allow changing between PressOnClick and PressOnRelease without pressing twice). Used by range_select branch."
...
defs["structs"]["ImGuiContext"][56]["comment"]["sameline"] = " // Was the value associated to the widget Edited over the course of the Active state."
...
defs["structs"]["ImGuiContext"][58]["comment"]["sameline"] = " // Clicked offset from upper-left corner, if applicable (currently only set by ButtonBehavior)"
...
defs["structs"]["ImGuiContext"][60]["comment"]["sameline"] = " // Activating with mouse or nav (gamepad/keyboard)"
...
defs["structs"]["ImGuiContext"][66]["comment"]["sameline"] = " // Store the last non-zero ActiveId, useful for animation."
...
defs["structs"]["ImGuiContext"][67]["comment"]["sameline"] = " // Store the last non-zero ActiveId timer since the beginning of activation, useful for animation."
...
defs["structs"]["ImGuiContext"][68]["comment"]["above"] = "    // Input Ownership"
defs["structs"]["ImGuiContext"][68]["comment"]["sameline"] = " // Active widget will want to read mouse wheel. Blocks scrolling the underlying window."
...
defs["structs"]["ImGuiContext"][69]["comment"]["sameline"] = " // Active widget will want to read those nav move requests (e.g. can activate a button and move away from it)"
...
defs["structs"]["ImGuiContext"][70]["comment"]["sameline"] = " // Active widget will want to read those nav inputs."
...
defs["structs"]["ImGuiContext"][71]["comment"]["sameline"] = " // Active widget will want to read those key inputs. When we grow the ImGuiKey enum we'll need to either to order the enum to make useful keys come first, either redesign this into e.g. a small array."
...
defs["structs"]["ImGuiContext"][72]["comment"]["above"] = "    // Next window/item data"
defs["structs"]["ImGuiContext"][72]["comment"]["sameline"] = " // == g.ItemFlagsStack.back()"
...
defs["structs"]["ImGuiContext"][73]["comment"]["sameline"] = " // Storage for SetNextItem** functions"
...
defs["structs"]["ImGuiContext"][74]["comment"]["sameline"] = " // Storage for last submitted item (setup by ItemAdd)"
...
defs["structs"]["ImGuiContext"][75]["comment"]["sameline"] = " // Storage for SetNextWindow** functions"
...
defs["structs"]["ImGuiContext"][76]["comment"]["above"] = "    // Shared stacks"
defs["structs"]["ImGuiContext"][76]["comment"]["sameline"] = " // Stack for PushStyleColor()/PopStyleColor() - inherited by Begin()"
...
defs["structs"]["ImGuiContext"][77]["comment"]["sameline"] = " // Stack for PushStyleVar()/PopStyleVar() - inherited by Begin()"
...
defs["structs"]["ImGuiContext"][78]["comment"]["sameline"] = " // Stack for PushFont()/PopFont() - inherited by Begin()"
...
defs["structs"]["ImGuiContext"][79]["comment"]["sameline"] = " // Stack for PushFocusScope()/PopFocusScope() - not inherited by Begin(), unless child window"
...
defs["structs"]["ImGuiContext"][80]["comment"]["sameline"] = " // Stack for PushItemFlag()/PopItemFlag() - inherited by Begin()"
...
defs["structs"]["ImGuiContext"][81]["comment"]["sameline"] = " // Stack for BeginGroup()/EndGroup() - not inherited by Begin()"
...
defs["structs"]["ImGuiContext"][82]["comment"]["sameline"] = " // Which popups are open (persistent)"
...
defs["structs"]["ImGuiContext"][83]["comment"]["sameline"] = " // Which level of BeginPopup() we are in (reset every frame)"
...
defs["structs"]["ImGuiContext"][85]["comment"]["above"] = "    // Viewports"
defs["structs"]["ImGuiContext"][85]["comment"]["sameline"] = " // Active viewports (always 1+, and generally 1 unless multi-viewports are enabled). Each viewports hold their copy of ImDrawData."
...
defs["structs"]["ImGuiContext"][86]["comment"]["sameline"] = " // == CurrentViewport->DpiScale"
...
defs["structs"]["ImGuiContext"][87]["comment"]["sameline"] = " // We track changes of viewport (happening in Begin) so we can call Platform_OnChangedViewport()"
...
defs["structs"]["ImGuiContext"][89]["comment"]["sameline"] = " // Last known viewport that was hovered by mouse (even if we are not hovering any viewport any more) + honoring the _NoInputs flag."
...
defs["structs"]["ImGuiContext"][91]["comment"]["sameline"] = " // Virtual monitor used as fallback if backend doesn't provide monitor information."
...
defs["structs"]["ImGuiContext"][92]["comment"]["sameline"] = " // Every time the front-most window changes, we stamp its viewport with an incrementing counter"
...
defs["structs"]["ImGuiContext"][93]["comment"]["above"] = "    // Gamepad/keyboard Navigation"
defs["structs"]["ImGuiContext"][93]["comment"]["sameline"] = " // Focused window for navigation. Could be called 'FocusedWindow'"
...
defs["structs"]["ImGuiContext"][94]["comment"]["sameline"] = " // Focused item for navigation"
...
defs["structs"]["ImGuiContext"][95]["comment"]["sameline"] = " // Identify a selection scope (selection code often wants to \"clear other items\" when landing on an item of the selection set)"
...
defs["structs"]["ImGuiContext"][96]["comment"]["sameline"] = " // ~~ (g.ActiveId == 0) && IsNavInputPressed(ImGuiNavInput_Activate) ? NavId : 0, also set when calling ActivateItem()"
...
defs["structs"]["ImGuiContext"][97]["comment"]["sameline"] = " // ~~ IsNavInputDown(ImGuiNavInput_Activate) ? NavId : 0"
...
defs["structs"]["ImGuiContext"][98]["comment"]["sameline"] = " // ~~ IsNavInputPressed(ImGuiNavInput_Activate) ? NavId : 0"
...
defs["structs"]["ImGuiContext"][99]["comment"]["sameline"] = " // ~~ IsNavInputPressed(ImGuiNavInput_Input) ? NavId : 0; ImGuiActivateFlags_PreferInput will be set and NavActivateId will be 0."
...
defs["structs"]["ImGuiContext"][101]["comment"]["sameline"] = " // Just navigated to this id (result of a successfully MoveRequest)."
...
defs["structs"]["ImGuiContext"][102]["comment"]["sameline"] = " // Just navigated to this focus scope id (result of a successfully MoveRequest)."
...
defs["structs"]["ImGuiContext"][104]["comment"]["sameline"] = " // Set by ActivateItem(), queued until next frame."
...
defs["structs"]["ImGuiContext"][106]["comment"]["sameline"] = " // Keyboard or Gamepad mode? THIS WILL ONLY BE None or NavGamepad or NavKeyboard."
...
defs["structs"]["ImGuiContext"][107]["comment"]["sameline"] = " // Layer we are navigating on. For now the system is hard-coded for 0=main contents and 1=menu/title bar, may expose layers later."
...
defs["structs"]["ImGuiContext"][108]["comment"]["sameline"] = " // Nav widget has been seen this frame ~~ NavRectRel is valid"
...
defs["structs"]["ImGuiContext"][109]["comment"]["sameline"] = " // When set we will update mouse position if (io.ConfigFlags & ImGuiConfigFlags_NavEnableSetMousePos) if set (NB: this not enabled by default)"
...
defs["structs"]["ImGuiContext"][110]["comment"]["sameline"] = " // When user starts using mouse, we hide gamepad/keyboard highlight (NB: but they are still available, which is why NavDisableHighlight isn't always != NavDisableMouseHover)"
...
defs["structs"]["ImGuiContext"][111]["comment"]["sameline"] = " // When user starts using gamepad/keyboard, we hide mouse hovering highlight until mouse is touched again."
...
defs["structs"]["ImGuiContext"][112]["comment"]["above"] = "    // Navigation: Init & Move Requests"
defs["structs"]["ImGuiContext"][112]["comment"]["sameline"] = " // ~~ NavMoveRequest || NavInitRequest this is to perform early out in ItemAdd()"
...
defs["structs"]["ImGuiContext"][113]["comment"]["sameline"] = " // Init request for appearing window to select first item"
...
defs["structs"]["ImGuiContext"][115]["comment"]["sameline"] = " // Init request result (first item of the window, or one for which SetItemDefaultFocus() was called)"
...
defs["structs"]["ImGuiContext"][116]["comment"]["sameline"] = " // Init request result rectangle (relative to parent window)"
...
defs["structs"]["ImGuiContext"][117]["comment"]["sameline"] = " // Move request submitted, will process result on next NewFrame()"
...
defs["structs"]["ImGuiContext"][118]["comment"]["sameline"] = " // Move request submitted, still scoring incoming items"
...
defs["structs"]["ImGuiContext"][123]["comment"]["sameline"] = " // Direction of the move request (left/right/up/down)"
...
defs["structs"]["ImGuiContext"][125]["comment"]["sameline"] = " // FIXME-NAV: Describe the purpose of this better. Might want to rename?"
...
defs["structs"]["ImGuiContext"][126]["comment"]["sameline"] = " // Rectangle used for scoring, in screen space. Based of window->NavRectRel[], modified for directional navigation scoring."
...
defs["structs"]["ImGuiContext"][127]["comment"]["sameline"] = " // Some nav operations (such as PageUp/PageDown) enforce a region which clipper will attempt to always keep submitted"
...
defs["structs"]["ImGuiContext"][128]["comment"]["sameline"] = " // Metrics for debugging"
...
defs["structs"]["ImGuiContext"][129]["comment"]["sameline"] = " // Generally -1 or +1, 0 when tabbing without a nav id"
...
defs["structs"]["ImGuiContext"][130]["comment"]["sameline"] = " // >0 when counting items for tabbing"
...
defs["structs"]["ImGuiContext"][131]["comment"]["sameline"] = " // Best move request candidate within NavWindow"
...
defs["structs"]["ImGuiContext"][132]["comment"]["sameline"] = " // Best move request candidate within NavWindow that are mostly visible (when using ImGuiNavMoveFlags_AlsoScoreVisibleSet flag)"
...
defs["structs"]["ImGuiContext"][133]["comment"]["sameline"] = " // Best move request candidate within NavWindow's flattened hierarchy (when using ImGuiWindowFlags_NavFlattened flag)"
...
defs["structs"]["ImGuiContext"][134]["comment"]["sameline"] = " // First tabbing request candidate within NavWindow and flattened hierarchy"
...
defs["structs"]["ImGuiContext"][135]["comment"]["above"] = "    // Navigation: Windowing (CTRL+TAB for list, or Menu button + keys or directional pads to move/resize)"
defs["structs"]["ImGuiContext"][135]["comment"]["sameline"] = " // Target window when doing CTRL+Tab (or Pad Menu + FocusPrev/Next), this window is temporarily displayed top-most!"
...
defs["structs"]["ImGuiContext"][136]["comment"]["sameline"] = " // Record of last valid NavWindowingTarget until DimBgRatio and NavWindowingHighlightAlpha becomes 0.0f, so the fade-out can stay on it."
...
defs["structs"]["ImGuiContext"][137]["comment"]["sameline"] = " // Internal window actually listing the CTRL+Tab contents"
...
defs["structs"]["ImGuiContext"][141]["comment"]["above"] = "    // Render"
defs["structs"]["ImGuiContext"][141]["comment"]["sameline"] = " // 0.0..1.0 animation when fading in a dimming background (for modal window and CTRL+TAB list)"
...
defs["structs"]["ImGuiContext"][143]["comment"]["above"] = "    // Drag and Drop"
...
defs["structs"]["ImGuiContext"][144]["comment"]["sameline"] = " // Set when within a BeginDragDropXXX/EndDragDropXXX block for a drag source."
...
defs["structs"]["ImGuiContext"][145]["comment"]["sameline"] = " // Set when within a BeginDragDropXXX/EndDragDropXXX block for a drag target."
...
defs["structs"]["ImGuiContext"][150]["comment"]["sameline"] = " // Store rectangle of current target candidate (we favor small targets when overlapping)"
...
defs["structs"]["ImGuiContext"][153]["comment"]["sameline"] = " // Target item surface (we resolve overlapping targets by prioritizing the smaller surface)"
...
defs["structs"]["ImGuiContext"][154]["comment"]["sameline"] = " // Target item id (set at the time of accepting the payload)"
...
defs["structs"]["ImGuiContext"][155]["comment"]["sameline"] = " // Target item id from previous frame (we need to store this to allow for overlapping drag and drop targets)"
...
defs["structs"]["ImGuiContext"][156]["comment"]["sameline"] = " // Last time a target expressed a desire to accept the source"
...
defs["structs"]["ImGuiContext"][157]["comment"]["sameline"] = " // Set when holding a payload just made ButtonBehavior() return a press."
...
defs["structs"]["ImGuiContext"][158]["comment"]["sameline"] = " // We don't expose the ImVector<> directly, ImGuiPayload only holds pointer+size"
...
defs["structs"]["ImGuiContext"][159]["comment"]["sameline"] = " // Local buffer for small payloads"
...
defs["structs"]["ImGuiContext"][160]["comment"]["above"] = "    // Clipper"
...
defs["structs"]["ImGuiContext"][162]["comment"]["above"] = "    // Tables"
...
defs["structs"]["ImGuiContext"][163]["comment"]["sameline"] = " // Temporary table data size (because we leave previous instances undestructed, we generally don't use TablesTempData.Size)"
...
defs["structs"]["ImGuiContext"][164]["comment"]["sameline"] = " // Temporary table data (buffers reused/shared across instances, support nesting)"
...
defs["structs"]["ImGuiContext"][165]["comment"]["sameline"] = " // Persistent table data"
...
defs["structs"]["ImGuiContext"][166]["comment"]["sameline"] = " // Last used timestamp of each tables (SOA, for efficient GC)"
...
defs["structs"]["ImGuiContext"][168]["comment"]["above"] = "    // Tab bars"
...
defs["structs"]["ImGuiContext"][172]["comment"]["above"] = "    // Widget state"
...
defs["structs"]["ImGuiContext"][175]["comment"]["sameline"] = " // Temporary text input when CTRL+clicking on a slider, etc."
...
defs["structs"]["ImGuiContext"][176]["comment"]["sameline"] = " // Store user options for color edit widgets"
...
defs["structs"]["ImGuiContext"][177]["comment"]["sameline"] = " // Backup of last Hue associated to LastColor, so we can restore Hue in lossy RGB<>HSV round trips"
...
defs["structs"]["ImGuiContext"][178]["comment"]["sameline"] = " // Backup of last Saturation associated to LastColor, so we can restore Saturation in lossy RGB<>HSV round trips"
...
defs["structs"]["ImGuiContext"][179]["comment"]["sameline"] = " // RGB value with alpha set to 0."
...
defs["structs"]["ImGuiContext"][180]["comment"]["sameline"] = " // Initial/reference color at the time of opening the color picker."
...
defs["structs"]["ImGuiContext"][183]["comment"]["sameline"] = " // Accumulated slider delta when using navigation controls."
...
defs["structs"]["ImGuiContext"][184]["comment"]["sameline"] = " // Has the accumulated slider delta changed since last time we tried to apply it?"
...
defs["structs"]["ImGuiContext"][186]["comment"]["sameline"] = " // Accumulator for dragging modification. Always high-precision, not rounded by end-user precision settings"
...
defs["structs"]["ImGuiContext"][187]["comment"]["sameline"] = " // If speed == 0.0f, uses (max-min) * DragSpeedDefaultRatio"
...
defs["structs"]["ImGuiContext"][188]["comment"]["sameline"] = " // Distance between mouse and center of grab box, normalized in parent space. Use storage?"
...
defs["structs"]["ImGuiContext"][189]["comment"]["sameline"] = " // Backup for style.Alpha for BeginDisabled()"
...
defs["structs"]["ImGuiContext"][192]["comment"]["sameline"] = " // Time before slow tooltips appears (FIXME: This is temporary until we merge in tooltip timer+priority work)"
...
defs["structs"]["ImGuiContext"][193]["comment"]["sameline"] = " // If no custom clipboard handler is defined"
...
defs["structs"]["ImGuiContext"][194]["comment"]["sameline"] = " // A list of menu IDs that were rendered at least once"
...
defs["structs"]["ImGuiContext"][195]["comment"]["above"] = "    // Platform support"
defs["structs"]["ImGuiContext"][195]["comment"]["sameline"] = " // Data updated by current frame"
...
defs["structs"]["ImGuiContext"][196]["comment"]["sameline"] = " // Previous frame data (when changing we will call io.SetPlatformImeDataFn"
...
defs["structs"]["ImGuiContext"][198]["comment"]["sameline"] = " // '.' or *localeconv()->decimal_point"
...
defs["structs"]["ImGuiContext"][199]["comment"]["above"] = "    // Extensions\
...
defs["structs"]["ImGuiContext"][200]["comment"]["above"] = "    // Settings"
...
defs["structs"]["ImGuiContext"][201]["comment"]["sameline"] = " // Save .ini Settings to memory when time reaches zero"
...
defs["structs"]["ImGuiContext"][202]["comment"]["sameline"] = " // In memory .ini settings"
...
defs["structs"]["ImGuiContext"][203]["comment"]["sameline"] = " // List of .ini settings handlers"
...
defs["structs"]["ImGuiContext"][204]["comment"]["sameline"] = " // ImGuiWindow .ini settings entries"
...
defs["structs"]["ImGuiContext"][205]["comment"]["sameline"] = " // ImGuiTable .ini settings entries"
...
defs["structs"]["ImGuiContext"][206]["comment"]["sameline"] = " // Hooks for extensions (e.g. test engine)"
...
defs["structs"]["ImGuiContext"][207]["comment"]["sameline"] = " // Next available HookId"
...
defs["structs"]["ImGuiContext"][208]["comment"]["above"] = "    // Capture/Logging"
defs["structs"]["ImGuiContext"][208]["comment"]["sameline"] = " // Currently capturing"
...
defs["structs"]["ImGuiContext"][209]["comment"]["sameline"] = " // Capture target"
...
defs["structs"]["ImGuiContext"][210]["comment"]["sameline"] = " // If != NULL log to stdout/ file"
...
defs["structs"]["ImGuiContext"][211]["comment"]["sameline"] = " // Accumulation buffer when log to clipboard. This is pointer so our GImGui static constructor doesn't call heap allocators."
...
defs["structs"]["ImGuiContext"][218]["comment"]["sameline"] = " // Default/stored value for LogDepthMaxExpand if not specified in the LogXXX function call."
...
defs["structs"]["ImGuiContext"][219]["comment"]["above"] = "    // Debug Tools"
...
defs["structs"]["ImGuiContext"][221]["comment"]["sameline"] = " // Item picker is active (started with DebugStartItemPicker())"
...
defs["structs"]["ImGuiContext"][222]["comment"]["sameline"] = " // Will call IM_DEBUG_BREAK() when encountering this ID"
...
defs["structs"]["ImGuiContext"][225]["comment"]["above"] = "    // Misc"
defs["structs"]["ImGuiContext"][225]["comment"]["sameline"] = " // Calculate estimate of framerate for user over the last 2 seconds."
...
defs["structs"]["ImGuiContext"][229]["comment"]["sameline"] = " // Explicit capture override via SetNextFrameWantCaptureMouse()/SetNextFrameWantCaptureKeyboard(). Default to -1."
...
defs["structs"]["ImGuiContext"][230]["comment"]["sameline"] = " // \""
...
defs["structs"]["ImGuiContext"][232]["comment"]["sameline"] = " // Temporary text buffer"
...
defs["structs"]["ImGuiDockNode"][31]["comment"]["sameline"] = " // Set to false when the node is hidden (usually disabled as it has no active window)"
...
defs["structs"]["ImGuiDockNode"][34]["comment"]["sameline"] = " // Provide space for a close button (if any of the docked window has one). Note that button may be hidden on window without one."
...
defs["structs"]["ImGuiDockNode"][37]["comment"]["sameline"] = " // Set when closing all tabs at once."
...
defs["structs"]["ImGuiDockNode"][39]["comment"]["sameline"] = " // After a node extraction we need to transition toward moving the newly created host window"
...
defs["structs"]["ImGuiIO"][8]["comment"]["sameline"] = " // = 0.30f          // Time for a double-click, in seconds."
...
defs["structs"]["ImGuiIO"][9]["comment"]["sameline"] = " // = 6.0f           // Distance threshold to stay in to validate a double-click, in pixels."
...
defs["structs"]["ImGuiIO"][10]["comment"]["sameline"] = " // = 6.0f           // Distance threshold before considering we are dragging."
...
defs["structs"]["ImGuiIO"][11]["comment"]["sameline"] = " // = 0.250f         // When holding a key/button, time before it starts repeating, in seconds (for buttons in Repeat mode, etc.)."
...
defs["structs"]["ImGuiIO"][12]["comment"]["sameline"] = " // = 0.050f         // When holding a key/button, rate at which it repeats, in seconds."
...
defs["structs"]["ImGuiIO"][13]["comment"]["sameline"] = " // = NULL           // Store your own data for retrieval by callbacks."
...
defs["structs"]["ImGuiIO"][14]["comment"]["sameline"] = " // <auto>           // Font atlas: load, rasterize and pack one or more fonts into a single texture."
...
defs["structs"]["ImGuiIO"][15]["comment"]["sameline"] = " // = 1.0f           // Global scale all fonts"
...
defs["structs"]["ImGuiIO"][16]["comment"]["sameline"] = " // = false          // Allow user scaling text of individual window with CTRL+Wheel."
...
defs["structs"]["ImGuiIO"][17]["comment"]["sameline"] = " // = NULL           // Font to use on NewFrame(). Use NULL to uses Fonts->Fonts[0]."
...
defs["structs"]["ImGuiIO"][18]["comment"]["sameline"] = " // = (1, 1)         // For retina display or other situations where window coordinates are different from framebuffer coordinates. This generally ends up in ImDrawData::FramebufferScale."
...
defs["structs"]["ImGuiIO"][19]["comment"]["above"] = "    // Docking options (when ImGuiConfigFlags_DockingEnable is set)"
defs["structs"]["ImGuiIO"][19]["comment"]["sameline"] = " // = false          // Simplified docking mode: disable window splitting, so docking is limited to merging multiple windows together into tab-bars."
...
defs["structs"]["ImGuiIO"][20]["comment"]["sameline"] = " // = false          // Enable docking with holding Shift key (reduce visual noise, allows dropping in wider space)"
...
defs["structs"]["ImGuiIO"][21]["comment"]["sameline"] = " // = false          // [BETA] [FIXME: This currently creates regression with auto-sizing and general overhead] Make every single floating window display within a docking node."
...
defs["structs"]["ImGuiIO"][22]["comment"]["sameline"] = "// = false          // [BETA] Make window or viewport transparent when docking and only display docking boxes on the target viewport. Useful if rendering of multiple viewport cannot be synced. Best used with ConfigViewportsNoAutoMerge."
...
defs["structs"]["ImGuiIO"][23]["comment"]["above"] = "    // Viewport options (when ImGuiConfigFlags_ViewportsEnable is set)"
defs["structs"]["ImGuiIO"][23]["comment"]["sameline"] = " // = false;         // Set to make all floating imgui windows always create their own viewport. Otherwise, they are merged into the main host viewports when overlapping it. May also set ImGuiViewportFlags_NoAutoMerge on individual viewport."
...
defs["structs"]["ImGuiIO"][24]["comment"]["sameline"] = " // = false          // Disable default OS task bar icon flag for secondary viewports. When a viewport doesn't want a task bar icon, ImGuiViewportFlags_NoTaskBarIcon will be set on it."
...
defs["structs"]["ImGuiIO"][25]["comment"]["sameline"] = " // = true           // Disable default OS window decoration flag for secondary viewports. When a viewport doesn't want window decorations, ImGuiViewportFlags_NoDecoration will be set on it. Enabling decoration can create subsequent issues at OS levels (e.g. minimum window size)."
...
defs["structs"]["ImGuiIO"][27]["comment"]["above"] = "    // Miscellaneous options"
defs["structs"]["ImGuiIO"][27]["comment"]["sameline"] = " // = false          // Request ImGui to draw a mouse cursor for you (if you are on a platform without a mouse cursor). Cannot be easily renamed to 'io.ConfigXXX' because this is frequently used by backend implementations."
...
defs["structs"]["ImGuiIO"][28]["comment"]["sameline"] = " // = defined(__APPLE__) // OS X style: Text editing cursor movement using Alt instead of Ctrl, Shortcuts using Cmd/Super instead of Ctrl, Line/Text Start and End using Cmd+Arrows instead of Home/End, Double click selects by word instead of selecting whole text, Multi-selection in lists uses Cmd/Super instead of Ctrl."
...
defs["structs"]["ImGuiIO"][29]["comment"]["sameline"] = " // = true           // Enable input queue trickling: some types of events submitted during the same frame (e.g. button down + up) will be spread over multiple frames, improving interactions with low framerates."
...
defs["structs"]["ImGuiIO"][30]["comment"]["sameline"] = " // = true           // Enable blinking cursor (optional as some users consider it to be distracting)."
...
defs["structs"]["ImGuiIO"][31]["comment"]["sameline"] = " // = false          // [BETA] Enable turning DragXXX widgets into text input with a simple mouse click-release (without moving). Not desirable on devices without a keyboard."
...
defs["structs"]["ImGuiIO"][32]["comment"]["sameline"] = " // = true           // Enable resizing of windows from their edges and from the lower-left corner. This requires (io.BackendFlags & ImGuiBackendFlags_HasMouseCursors) because it needs mouse cursor feedback. (This used to be a per-window ImGuiWindowFlags_ResizeFromAnySide flag)"
...
defs["structs"]["ImGuiIO"][33]["comment"]["sameline"] = " // = false       // Enable allowing to move windows only when clicking on their title bar. Does not apply to windows without a title bar."
...
defs["structs"]["ImGuiIO"][34]["comment"]["sameline"] = " // = 60.0f          // Timer (in seconds) to free transient windows/tables memory buffers when unused. Set to -1.0f to disable."
...
defs["structs"]["ImGuiIO"][35]["comment"]["above"] = "    // Optional: Platform/Renderer backend name (informational only! will be displayed in About Window) + User data for backend/wrappers to store their own stuff."
defs["structs"]["ImGuiIO"][35]["comment"]["sameline"] = " // = NULL"
...
defs["structs"]["ImGuiIO"][36]["comment"]["sameline"] = " // = NULL"
...
defs["structs"]["ImGuiIO"][37]["comment"]["sameline"] = " // = NULL           // User data for platform backend"
...
defs["structs"]["ImGuiIO"][38]["comment"]["sameline"] = " // = NULL           // User data for renderer backend"
...
defs["structs"]["ImGuiIO"][39]["comment"]["sameline"] = " // = NULL           // User data for non C++ programming language backend"
...
defs["structs"]["ImGuiIO"][40]["comment"]["above"] = "    // Optional: Access OS clipboard\
...
defs["structs"]["ImGuiIO"][43]["comment"]["above"] = "    // Optional: Notify OS Input Method Editor of the screen position of your cursor for text input position (e.g. when using Japanese/Chinese IME on Windows)\
...
defs["structs"]["ImGuiIO"][44]["comment"]["sameline"] = " // Unused field to keep data structure the same size."
...
defs["structs"]["ImGuiIO"][45]["comment"]["sameline"] = " // Set when Dear ImGui will use mouse inputs, in this case do not dispatch them to your main game/application (either way, always pass on mouse inputs to imgui). (e.g. unclicked mouse is hovering over an imgui window, widget is active, mouse was clicked over an imgui window, etc.)."
...
defs["structs"]["ImGuiIO"][46]["comment"]["sameline"] = " // Set when Dear ImGui will use keyboard inputs, in this case do not dispatch them to your main game/application (either way, always pass keyboard inputs to imgui). (e.g. InputText active, or an imgui window is focused and navigation is enabled, etc.)."
...
defs["structs"]["ImGuiIO"][47]["comment"]["sameline"] = " // Mobile/console: when set, you may display an on-screen keyboard. This is set by Dear ImGui when it wants textual keyboard input to happen (e.g. when a InputText widget is active)."
...
defs["structs"]["ImGuiIO"][48]["comment"]["sameline"] = " // MousePos has been altered, backend should reposition mouse on next frame. Rarely used! Set only when ImGuiConfigFlags_NavEnableSetMousePos flag is enabled."
...
defs["structs"]["ImGuiIO"][49]["comment"]["sameline"] = " // When manual .ini load/save is active (io.IniFilename == NULL), this will be set to notify your application that you can call SaveIniSettingsToMemory() and save yourself. Important: clear io.WantSaveIniSettings yourself after saving!"
...
defs["structs"]["ImGuiIO"][50]["comment"]["sameline"] = " // Keyboard/Gamepad navigation is currently allowed (will handle ImGuiKey_NavXXX events) = a window is focused and it doesn't use the ImGuiWindowFlags_NoNavInputs flag."
...
defs["structs"]["ImGuiIO"][51]["comment"]["sameline"] = " // Keyboard/Gamepad navigation is visible and allowed (will handle ImGuiKey_NavXXX events)."
...
defs["structs"]["ImGuiIO"][52]["comment"]["sameline"] = " // Rough estimate of application framerate, in frame per second. Solely for convenience. Rolling average estimation based on io.DeltaTime over 120 frames."
...
defs["structs"]["ImGuiIO"][53]["comment"]["sameline"] = " // Vertices output during last call to Render()"
...
defs["structs"]["ImGuiIO"][54]["comment"]["sameline"] = " // Indices output during last call to Render() = number of triangles * 3"
...
defs["structs"]["ImGuiIO"][55]["comment"]["sameline"] = " // Number of visible windows"
...
defs["structs"]["ImGuiIO"][56]["comment"]["sameline"] = " // Number of active windows"
...
defs["structs"]["ImGuiIO"][57]["comment"]["sameline"] = " // Number of active allocations, updated by MemAlloc/MemFree based on current context. May be off if you have multiple imgui contexts."
...
defs["structs"]["ImGuiIO"][58]["comment"]["sameline"] = " // Mouse delta. Note that this is zero if either current or previous position are invalid (-FLT_MAX,-FLT_MAX), so a disappearing/reappearing mouse won't have a huge delta."
...
defs["structs"]["ImGuiIO"][59]["comment"]["sameline"] = " // [LEGACY] Input: map of indices into the KeysDown[512] entries array which represent your \"native\" keyboard state. The first 512 are now unused and should be kept zero. Legacy backend will write into KeyMap[] using ImGuiKey_ indices which are always >512."
...
defs["structs"]["ImGuiIO"][60]["comment"]["sameline"] = " // [LEGACY] Input: Keyboard keys that are pressed (ideally left in the \"native\" order your engine has access to keyboard keys, so you can use your own defines/enums for keys). This used to be [512] sized. It is now ImGuiKey_COUNT to allow legacy io.KeysDown[GetKeyIndex(...)] to work without an overflow."
...
defs["structs"]["ImGuiIO"][61]["comment"]["above"] = "    // Main Input State\
    // (this block used to be written by backend, since 1.87 it is best to NOT write to those directly, call the AddXXX functions above instead)\
    // (reading from those variables is fair game, as they are extremely unlikely to be moving anywhere)"
defs["structs"]["ImGuiIO"][61]["comment"]["sameline"] = " // Mouse position, in pixels. Set to ImVec2(-FLT_MAX, -FLT_MAX) if mouse is unavailable (on another screen, etc.)"
...
defs["structs"]["ImGuiIO"][62]["comment"]["sameline"] = " // Mouse buttons: 0=left, 1=right, 2=middle + extras (ImGuiMouseButton_COUNT == 5). Dear ImGui mostly uses left and right buttons. Others buttons allows us to track if the mouse is being used by your application + available to user as a convenience via IsMouse** API."
...
defs["structs"]["ImGuiIO"][63]["comment"]["sameline"] = " // Mouse wheel Vertical: 1 unit scrolls about 5 lines text."
...
defs["structs"]["ImGuiIO"][64]["comment"]["sameline"] = " // Mouse wheel Horizontal. Most users don't have a mouse with an horizontal wheel, may not be filled by all backends."
...
defs["structs"]["ImGuiIO"][65]["comment"]["sameline"] = " // (Optional) Modify using io.AddMouseViewportEvent(). With multi-viewports: viewport the OS mouse is hovering. If possible _IGNORING_ viewports with the ImGuiViewportFlags_NoInputs flag is much better (few backends can handle that). Set io.BackendFlags |= ImGuiBackendFlags_HasMouseHoveredViewport if you can provide this info. If you don't imgui will infer the value using the rectangles and last focused time of the viewports it knows about (ignoring other OS windows)."
...
defs["structs"]["ImGuiIO"][66]["comment"]["sameline"] = " // Keyboard modifier down: Control"
...
defs["structs"]["ImGuiIO"][67]["comment"]["sameline"] = " // Keyboard modifier down: Shift"
...
defs["structs"]["ImGuiIO"][68]["comment"]["sameline"] = " // Keyboard modifier down: Alt"
...
defs["structs"]["ImGuiIO"][69]["comment"]["sameline"] = " // Keyboard modifier down: Cmd/Super/Windows"
...
defs["structs"]["ImGuiIO"][70]["comment"]["sameline"] = " // Gamepad inputs. Cleared back to zero by EndFrame(). Keyboard keys will be auto-mapped and be written here by NewFrame()."
...
defs["structs"]["ImGuiIO"][71]["comment"]["above"] = "    // Other state maintained from data above + IO function calls"
defs["structs"]["ImGuiIO"][71]["comment"]["sameline"] = " // Key mods flags (same as io.KeyCtrl/KeyShift/KeyAlt/KeySuper but merged into flags), updated by NewFrame()"
...
defs["structs"]["ImGuiIO"][72]["comment"]["sameline"] = " // Key state for all known keys. Use IsKeyXXX() functions to access this."
...
defs["structs"]["ImGuiIO"][73]["comment"]["sameline"] = " // Alternative to WantCaptureMouse: (WantCaptureMouse == true && WantCaptureMouseUnlessPopupClose == false) when a click over void is expected to close a popup."
...
defs["structs"]["ImGuiIO"][74]["comment"]["sameline"] = " // Previous mouse position (note that MouseDelta is not necessary == MousePos-MousePosPrev, in case either position is invalid)"
...
defs["structs"]["ImGuiIO"][75]["comment"]["sameline"] = " // Position at time of clicking"
...
defs["structs"]["ImGuiIO"][76]["comment"]["sameline"] = " // Time of last click (used to figure out double-click)"
...
defs["structs"]["ImGuiIO"][77]["comment"]["sameline"] = " // Mouse button went from !Down to Down (same as MouseClickedCount[x] != 0)"
...
defs["structs"]["ImGuiIO"][78]["comment"]["sameline"] = " // Has mouse button been double-clicked? (same as MouseClickedCount[x] == 2)"
...
defs["structs"]["ImGuiIO"][79]["comment"]["sameline"] = " // == 0 (not clicked), == 1 (same as MouseClicked[]), == 2 (double-clicked), == 3 (triple-clicked) etc. when going from !Down to Down"
...
defs["structs"]["ImGuiIO"][80]["comment"]["sameline"] = " // Count successive number of clicks. Stays valid after mouse release. Reset after another click is done."
...
defs["structs"]["ImGuiIO"][81]["comment"]["sameline"] = " // Mouse button went from Down to !Down"
...
defs["structs"]["ImGuiIO"][82]["comment"]["sameline"] = " // Track if button was clicked inside a dear imgui window or over void blocked by a popup. We don't request mouse capture from the application if click started outside ImGui bounds."
...
defs["structs"]["ImGuiIO"][83]["comment"]["sameline"] = " // Track if button was clicked inside a dear imgui window."
...
defs["structs"]["ImGuiIO"][84]["comment"]["sameline"] = " // Duration the mouse button has been down (0.0f == just clicked)"
...
defs["structs"]["ImGuiIO"][85]["comment"]["sameline"] = " // Previous time the mouse button has been down"
...
defs["structs"]["ImGuiIO"][86]["comment"]["sameline"] = " // Maximum distance, absolute, on each axis, of how much mouse has traveled from the clicking point"
...
defs["structs"]["ImGuiIO"][87]["comment"]["sameline"] = " // Squared maximum distance of how much mouse has traveled from the clicking point (used for moving thresholds)"
...
defs["structs"]["ImGuiIO"][90]["comment"]["sameline"] = " // Touch/Pen pressure (0.0f to 1.0f, should be >0.0f only when MouseDown[0] == true). Helper storage currently unused by Dear ImGui."
...
defs["structs"]["ImGuiIO"][91]["comment"]["sameline"] = " // Only modify via AddFocusEvent()"
...
defs["structs"]["ImGuiIO"][92]["comment"]["sameline"] = " // Only modify via SetAppAcceptingEvents()"
...
defs["structs"]["ImGuiIO"][93]["comment"]["sameline"] = " // -1: unknown, 0: using AddKeyEvent(), 1: using legacy io.KeysDown[]"
...
defs["structs"]["ImGuiIO"][94]["comment"]["sameline"] = " // 0: using AddKeyAnalogEvent(), 1: writing to legacy io.NavInputs[] directly"
...
defs["structs"]["ImGuiIO"][95]["comment"]["sameline"] = " // For AddInputCharacterUTF16()"
...
defs["structs"]["ImGuiIO"][96]["comment"]["sameline"] = " // Queue of _characters_ input (obtained by platform backend). Fill using AddInputCharacter() helper."
...
defs["structs"]["ImGuiInputTextCallbackData"][1]["comment"]["sameline"] = " // One ImGuiInputTextFlags_Callback*    // Read-only"
...
defs["structs"]["ImGuiInputTextCallbackData"][2]["comment"]["sameline"] = " // What user passed to InputText()      // Read-only"
...
defs["structs"]["ImGuiInputTextCallbackData"][4]["comment"]["above"] = "    // Arguments for the different callback events\
    // - To modify the text buffer in a callback, prefer using the InsertChars() / DeleteChars() function. InsertChars() will take care of calling the resize callback if necessary.\
    // - If you know your edits are not going to resize the underlying buffer allocation, you may modify the contents of 'Buf[]' directly. You need to update 'BufTextLen' accordingly (0 <= BufTextLen < BufSize) and set 'BufDirty'' to true so InputText can update its internal state."
defs["structs"]["ImGuiInputTextCallbackData"][4]["comment"]["sameline"] = " // Character input                      // Read-write   // [CharFilter] Replace character with another one, or set to zero to drop. return 1 is equivalent to setting EventChar=0;"
...
defs["structs"]["ImGuiInputTextCallbackData"][5]["comment"]["sameline"] = " // Key pressed (Up/Down/TAB)            // Read-only    // [Completion,History]"
...
defs["structs"]["ImGuiInputTextCallbackData"][6]["comment"]["sameline"] = " // Text buffer                          // Read-write   // [Resize] Can replace pointer / [Completion,History,Always] Only write to pointed data, don't replace the actual pointer!"
...
defs["structs"]["ImGuiInputTextCallbackData"][7]["comment"]["sameline"] = " // Text length (in bytes)               // Read-write   // [Resize,Completion,History,Always] Exclude zero-terminator storage. In C land: == strlen(some_text), in C++ land: string.length()"
...
defs["structs"]["ImGuiInputTextCallbackData"][8]["comment"]["sameline"] = " // Buffer size (in bytes) = capacity+1  // Read-only    // [Resize,Completion,History,Always] Include zero-terminator storage. In C land == ARRAYSIZE(my_char_array), in C++ land: string.capacity()+1"
...
defs["structs"]["ImGuiInputTextCallbackData"][9]["comment"]["sameline"] = " // Set if you modify Buf/BufTextLen!    // Write        // [Completion,History,Always]"
...
defs["structs"]["ImGuiInputTextCallbackData"][10]["comment"]["sameline"] = " //                                      // Read-write   // [Completion,History,Always]"
...
defs["structs"]["ImGuiInputTextCallbackData"][11]["comment"]["sameline"] = " //                                      // Read-write   // [Completion,History,Always] == to SelectionEnd when no selection)"
...
defs["structs"]["ImGuiInputTextCallbackData"][12]["comment"]["sameline"] = " //                                      // Read-write   // [Completion,History,Always]"
...
defs["structs"]["ImGuiInputTextState"][1]["comment"]["sameline"] = " // widget id owning the text state"
...
defs["structs"]["ImGuiInputTextState"][2]["comment"]["sameline"] = " // we need to maintain our buffer length in both UTF-8 and wchar format. UTF-8 length is valid even if TextA is not."
...
defs["structs"]["ImGuiInputTextState"][4]["comment"]["sameline"] = " // edit buffer, we need to persist but can't guarantee the persistence of the user-provided buffer. so we copy into own buffer."
...
defs["structs"]["ImGuiInputTextState"][5]["comment"]["sameline"] = " // temporary UTF8 buffer for callbacks and other operations. this is not updated in every code-path! size=capacity."
...
defs["structs"]["ImGuiInputTextState"][6]["comment"]["sameline"] = " // backup of end-user buffer at the time of focus (in UTF-8, unaltered)"
...
defs["structs"]["ImGuiInputTextState"][7]["comment"]["sameline"] = " // temporary UTF8 buffer is not initially valid before we make the widget active (until then we pull the data from user argument)"
...
defs["structs"]["ImGuiInputTextState"][8]["comment"]["sameline"] = " // end-user buffer capacity"
...
defs["structs"]["ImGuiInputTextState"][9]["comment"]["sameline"] = " // horizontal scrolling/offset"
...
defs["structs"]["ImGuiInputTextState"][10]["comment"]["sameline"] = " // state for stb_textedit.h"
...
defs["structs"]["ImGuiInputTextState"][11]["comment"]["sameline"] = " // timer for cursor blink, reset on every user action so the cursor reappears immediately"
...
defs["structs"]["ImGuiInputTextState"][12]["comment"]["sameline"] = " // set when we want scrolling to follow the current cursor position (not always!)"
...
defs["structs"]["ImGuiInputTextState"][13]["comment"]["sameline"] = " // after a double-click to select all, we ignore further mouse drags to update selection"
...
defs["structs"]["ImGuiInputTextState"][14]["comment"]["sameline"] = " // edited this frame"
...
defs["structs"]["ImGuiInputTextState"][15]["comment"]["sameline"] = " // copy of InputText() flags"
...
defs["structs"]["ImGuiLastItemData"][6]["comment"]["sameline"] = " // Display rectangle (only if ImGuiItemStatusFlags_HasDisplayRect is set)"
...
defs["structs"]["ImGuiListClipper"][1]["comment"]["sameline"] = " // First item to display, updated by each call to Step()"
...
defs["structs"]["ImGuiListClipper"][2]["comment"]["sameline"] = " // End of items to display (exclusive)"
...
defs["structs"]["ImGuiListClipper"][3]["comment"]["sameline"] = " // [Internal] Number of items"
...
defs["structs"]["ImGuiListClipper"][4]["comment"]["sameline"] = " // [Internal] Height of item after a first step and item submission can calculate it"
...
defs["structs"]["ImGuiListClipper"][5]["comment"]["sameline"] = " // [Internal] Cursor position at the time of Begin() or after table frozen rows are all processed"
...
defs["structs"]["ImGuiListClipper"][6]["comment"]["sameline"] = " // [Internal] Internal data"
...
defs["structs"]["ImGuiNavItemData"][6]["comment"]["sameline"] = " //      Move    // Best candidate box distance to current NavId"
...
defs["structs"]["ImGuiNavItemData"][7]["comment"]["sameline"] = " //      Move    // Best candidate center distance to current NavId"
...
defs["structs"]["ImGuiNavItemData"][8]["comment"]["sameline"] = " //      Move    // Best candidate axial distance to current NavId"
...
defs["structs"]["ImGuiNextItemData"][2]["comment"]["sameline"] = " // Set by SetNextItemWidth()"
...
defs["structs"]["ImGuiNextItemData"][3]["comment"]["sameline"] = " // Set by SetNextItemMultiSelectData() (!= 0 signify value has been set, so it's an alternate version of HasSelectionData, we don't use Flags for this because they are cleared too early. This is mostly used for debugging)"
...
defs["structs"]["ImGuiNextWindowData"][16]["comment"]["sameline"] = " // Override background alpha"
...
defs["structs"]["ImGuiNextWindowData"][20]["comment"]["sameline"] = " // (Always on) This is not exposed publicly, so we don't clear it and it doesn't have a corresponding flag (could we? for consistency?)"
...
defs["structs"]["ImGuiPopupData"][3]["comment"]["sameline"] = " // Set on OpenPopup() copy of NavWindow at the time of opening the popup"
...
defs["structs"]["ImGuiSizeCallbackData"][1]["comment"]["sameline"] = " // Read-only.   What user passed to SetNextWindowSizeConstraints()"
...
defs["structs"]["ImGuiStackTool"][2]["comment"]["sameline"] = " // -1: query stack and resize Results, >= 0: individual stack level"
...
defs["structs"]["ImGuiStackTool"][3]["comment"]["sameline"] = " // ID to query details for"
...
defs["structs"]["ImGuiStyle"][6]["comment"]["sameline"] = " // Minimum window size. This is a global setting. If you want to constraint individual windows, use SetNextWindowSizeConstraints()."
...
defs["structs"]["ImGuiStyle"][18]["comment"]["sameline"] = " // Padding within a table cell"
...
defs["structs"]["ImGuiStyle"][29]["comment"]["sameline"] = " // Minimum width for close button to appears on an unselected tab when hovered. Set to 0.0f to always show when hovering, set to FLT_MAX to never show close button unless selected."
...
defs["structs"]["ImGuiStyle"][30]["comment"]["sameline"] = " // Side of the color button in the ColorEdit4 widget (left/right). Defaults to ImGuiDir_Right."
...
defs["structs"]["ImGuiStyle"][31]["comment"]["sameline"] = " // Alignment of button text when button is larger than text. Defaults to (0.5f, 0.5f) (centered)."
...
defs["structs"]["ImGuiStyle"][32]["comment"]["sameline"] = " // Alignment of selectable text. Defaults to (0.0f, 0.0f) (top-left aligned). It's generally important to keep this left-aligned if you want to lay multiple items on a same line."
...
defs["structs"]["ImGuiStyle"][33]["comment"]["sameline"] = " // Window position are clamped to be visible within the display area or monitors by at least this amount. Only applies to regular windows."
...
defs["structs"]["ImGuiStyle"][34]["comment"]["sameline"] = " // If you cannot see the edges of your screen (e.g. on a TV) increase the safe area padding. Apply to popups/tooltips as well regular windows. NB: Prefer configuring your TV sets correctly!"
...
defs["structs"]["ImGuiStyle"][35]["comment"]["sameline"] = " // Scale software rendered mouse cursor (when io.MouseDrawCursor is enabled). We apply per-monitor DPI scaling over this scale. May be removed later."
...
defs["structs"]["ImGuiStyle"][36]["comment"]["sameline"] = " // Enable anti-aliased lines/borders. Disable if you are really tight on CPU/GPU. Latched at the beginning of the frame (copied to ImDrawList)."
...
defs["structs"]["ImGuiStyle"][37]["comment"]["sameline"] = " // Enable anti-aliased lines/borders using textures where possible. Require backend to render with bilinear filtering (NOT point/nearest filtering). Latched at the beginning of the frame (copied to ImDrawList)."
...
defs["structs"]["ImGuiStyle"][38]["comment"]["sameline"] = " // Enable anti-aliased edges around filled shapes (rounded rectangles, circles, etc.). Disable if you are really tight on CPU/GPU. Latched at the beginning of the frame (copied to ImDrawList)."
...
defs["structs"]["ImGuiStyle"][39]["comment"]["sameline"] = " // Tessellation tolerance when using PathBezierCurveTo() without a specific number of segments. Decrease for highly tessellated curves (higher quality, more polygons), increase to reduce quality."
...
defs["structs"]["ImGuiStyle"][40]["comment"]["sameline"] = " // Maximum error (in pixels) allowed when using AddCircle()/AddCircleFilled() or drawing rounded corner rectangles with no explicit segment count specified. Decrease for higher quality but more geometry."
...
defs["structs"]["ImGuiTabBar"][25]["comment"]["sameline"] = " // Set to true when a new tab item or button has been added to the tab bar during last frame"
...
defs["structs"]["ImGuiTabBar"][26]["comment"]["sameline"] = " // Number of tabs submitted this frame."
...
defs["structs"]["ImGuiTabBar"][27]["comment"]["sameline"] = " // Index of last BeginTabItem() tab for use by EndTabItem()"
...
defs["structs"]["ImGuiTabBar"][29]["comment"]["sameline"] = " // style.FramePadding locked at the time of BeginTabBar()"
...
defs["structs"]["ImGuiTabBar"][31]["comment"]["sameline"] = " // For non-docking tab bar we re-append names in a contiguous buffer."
...
defs["structs"]["ImGuiTabItem"][12]["comment"]["sameline"] = " // Index only used during TabBarLayout()"
...
defs["structs"]["ImGuiTable"][11]["comment"]["sameline"] = " // Column Index -> IsVisible || AutoFit (== expect user to submit items)"
...
defs["structs"]["ImGuiTable"][12]["comment"]["sameline"] = " // Which data were loaded from the .ini file (e.g. when order is not altered we won't save order)"
...
defs["structs"]["ImGuiTable"][13]["comment"]["sameline"] = " // Offset in g.SettingsTables"
...
defs["structs"]["ImGuiTable"][15]["comment"]["sameline"] = " // Number of columns declared in BeginTable()"
...
defs["structs"]["ImGuiTable"][18]["comment"]["sameline"] = " // Count of BeginTable() calls with same ID in the same frame (generally 0). This is a little bit similar to BeginCount for a window, but multiple table with same ID look are multiple tables, they are just synched."
...
defs["structs"]["ImGuiTable"][19]["comment"]["sameline"] = " // Mark which instance (generally 0) of the same ID is being interacted with"
...
defs["structs"]["ImGuiTable"][22]["comment"]["sameline"] = " // Height submitted to TableNextRow()"
...
defs["structs"]["ImGuiTable"][36]["comment"]["sameline"] = " // Padding from each borders"
...
defs["structs"]["ImGuiTable"][38]["comment"]["sameline"] = " // Spacing between non-bordered cells"
...
defs["structs"]["ImGuiTable"][40]["comment"]["sameline"] = " // User value passed to BeginTable(), see comments at the top of BeginTable() for details."
...
defs["structs"]["ImGuiTable"][41]["comment"]["sameline"] = " // Sum of current column width"
...
defs["structs"]["ImGuiTable"][42]["comment"]["sameline"] = " // Sum of ideal column width in order nothing to be clipped, used for auto-fitting and content width submission in outer window"
...
defs["structs"]["ImGuiTable"][43]["comment"]["sameline"] = " // Sum of weight of all enabled stretching columns"
...
defs["structs"]["ImGuiTable"][45]["comment"]["sameline"] = " // Lock minimum contents width while resizing down in order to not create feedback loops. But we allow growing the table."
...
defs["structs"]["ImGuiTable"][46]["comment"]["sameline"] = " // Reference scale to be able to rescale columns on font/dpi changes."
...
defs["structs"]["ImGuiTable"][47]["comment"]["sameline"] = " // Note: for non-scrolling table, OuterRect.Max.y is often FLT_MAX until EndTable(), unless a height has been specified in BeginTable()."
...
defs["structs"]["ImGuiTable"][48]["comment"]["sameline"] = " // InnerRect but without decoration. As with OuterRect, for non-scrolling tables, InnerRect.Max.y is"
...
defs["structs"]["ImGuiTable"][51]["comment"]["sameline"] = " // We use this to cpu-clip cell background color fill, evolve during the frame as we cross frozen rows boundaries"
...
defs["structs"]["ImGuiTable"][52]["comment"]["sameline"] = " // Actual ImDrawCmd clip rect for BG0/1 channel. This tends to be == OuterWindow->ClipRect at BeginTable() because output in BG0/BG1 is cpu-clipped"
...
defs["structs"]["ImGuiTable"][53]["comment"]["sameline"] = " // Actual ImDrawCmd clip rect for BG2 channel. This tends to be a correct, tight-fit, because output to BG2 are done by widgets relying on regular ClipRect."
...
defs["structs"]["ImGuiTable"][54]["comment"]["sameline"] = " // This is used to check if we can eventually merge our columns draw calls into the current draw call of the current window."
...
defs["structs"]["ImGuiTable"][55]["comment"]["sameline"] = " // Backup of InnerWindow->ClipRect during PushTableBackground()/PopTableBackground()"
...
defs["structs"]["ImGuiTable"][56]["comment"]["sameline"] = " // Parent window for the table"
...
defs["structs"]["ImGuiTable"][57]["comment"]["sameline"] = " // Window holding the table data (== OuterWindow or a child window)"
...
defs["structs"]["ImGuiTable"][58]["comment"]["sameline"] = " // Contiguous buffer holding columns names"
...
defs["structs"]["ImGuiTable"][59]["comment"]["sameline"] = " // Shortcut to TempData->DrawSplitter while in table. Isolate draw commands per columns to avoid switching clip rect constantly"
...
defs["structs"]["ImGuiTable"][61]["comment"]["sameline"] = " // FIXME-OPT: Using a small-vector pattern would be good."
...
defs["structs"]["ImGuiTable"][63]["comment"]["sameline"] = " // FIXME-OPT: Using a small-vector pattern would be good."
...
defs["structs"]["ImGuiTable"][64]["comment"]["sameline"] = " // Public facing sorts specs, this is what we return in TableGetSortSpecs()"
...
defs["structs"]["ImGuiTable"][66]["comment"]["sameline"] = " // Number of enabled columns (<= ColumnsCount)"
...
defs["structs"]["ImGuiTable"][68]["comment"]["sameline"] = " // Count calls to TableSetupColumn()"
...
defs["structs"]["ImGuiTable"][69]["comment"]["sameline"] = " // Index of column whose visible region is being hovered. Important: == ColumnsCount when hovering empty region after the right-most column!"
...
defs["structs"]["ImGuiTable"][70]["comment"]["sameline"] = " // Index of column whose right-border is being hovered (for resizing)."
...
defs["structs"]["ImGuiTable"][71]["comment"]["sameline"] = " // Index of single column requesting auto-fit."
...
defs["structs"]["ImGuiTable"][72]["comment"]["sameline"] = " // Index of column being resized. Reset when InstanceCurrent==0."
...
defs["structs"]["ImGuiTable"][73]["comment"]["sameline"] = " // Index of column being resized from previous frame."
...
defs["structs"]["ImGuiTable"][74]["comment"]["sameline"] = " // Index of column header being held."
...
defs["structs"]["ImGuiTable"][75]["comment"]["sameline"] = " // Index of column being reordered. (not cleared)"
...
defs["structs"]["ImGuiTable"][76]["comment"]["sameline"] = " // -1 or +1"
...
defs["structs"]["ImGuiTable"][77]["comment"]["sameline"] = " // Index of left-most non-hidden column."
...
defs["structs"]["ImGuiTable"][78]["comment"]["sameline"] = " // Index of right-most non-hidden column."
...
defs["structs"]["ImGuiTable"][79]["comment"]["sameline"] = " // Index of left-most stretched column."
...
defs["structs"]["ImGuiTable"][80]["comment"]["sameline"] = " // Index of right-most stretched column."
...
defs["structs"]["ImGuiTable"][81]["comment"]["sameline"] = " // Column right-clicked on, of -1 if opening context menu from a neutral/empty spot"
...
defs["structs"]["ImGuiTable"][82]["comment"]["sameline"] = " // Requested frozen rows count"
...
defs["structs"]["ImGuiTable"][83]["comment"]["sameline"] = " // Actual frozen row count (== FreezeRowsRequest, or == 0 when no scrolling offset)"
...
defs["structs"]["ImGuiTable"][84]["comment"]["sameline"] = " // Requested frozen columns count"
...
defs["structs"]["ImGuiTable"][85]["comment"]["sameline"] = " // Actual frozen columns count (== FreezeColumnsRequest, or == 0 when no scrolling offset)"
...
defs["structs"]["ImGuiTable"][86]["comment"]["sameline"] = " // Index of current RowCellData[] entry in current row"
...
defs["structs"]["ImGuiTable"][87]["comment"]["sameline"] = " // Redirect non-visible columns here."
...
defs["structs"]["ImGuiTable"][88]["comment"]["sameline"] = " // For Selectable() and other widgets drawing across columns after the freezing line. Index within DrawSplitter.Channels[]"
...
defs["structs"]["ImGuiTable"][90]["comment"]["sameline"] = " // Set by TableUpdateLayout() which is called when beginning the first row."
...
defs["structs"]["ImGuiTable"][91]["comment"]["sameline"] = " // Set when inside TableBeginRow()/TableEndRow()."
...
defs["structs"]["ImGuiTable"][94]["comment"]["sameline"] = " // Set when the first row had the ImGuiTableRowFlags_Headers flag."
...
defs["structs"]["ImGuiTable"][95]["comment"]["sameline"] = " // Set when default context menu is open (also see: ContextPopupColumn, InstanceInteracted)."
...
defs["structs"]["ImGuiTable"][97]["comment"]["sameline"] = " // Set when table settings have changed and needs to be reported into ImGuiTableSetttings data."
...
defs["structs"]["ImGuiTable"][98]["comment"]["sameline"] = " // Set when display order is unchanged from default (DisplayOrder contains 0...Count-1)"
...
defs["structs"]["ImGuiTable"][101]["comment"]["sameline"] = " // Set when we got past the frozen row."
...
defs["structs"]["ImGuiTable"][102]["comment"]["sameline"] = " // Set if user didn't explicitly set a sizing policy in BeginTable()"
...
defs["structs"]["ImGuiTable"][104]["comment"]["sameline"] = " // Backup of InnerWindow->SkipItem at the end of BeginTable(), because we will overwrite InnerWindow->SkipItem on a per-column basis"
...
defs["structs"]["ImGuiTableColumn"][41]["comment"]["sameline"] = " // Ordered of available sort directions (2-bits each)"
...
defs["structs"]["ImGuiTableColumnSortSpecs"][4]["comment"]["sameline"] = " // ImGuiSortDirection_Ascending or ImGuiSortDirection_Descending (you can use this or SortSign, whichever is more convenient for your sort function)"
...
defs["structs"]["ImGuiTableInstanceData"][1]["comment"]["sameline"] = " // Outer height from last frame // FIXME: multi-instance issue (#3955)"
...
defs["structs"]["ImGuiTableInstanceData"][2]["comment"]["sameline"] = " // Height of first row from last frame // FIXME: possible multi-instance issue?"
...
defs["structs"]["ImGuiTableTempData"][3]["comment"]["sameline"] = " // outer_size.x passed to BeginTable()"
...
defs["structs"]["ImGuiTableTempData"][5]["comment"]["sameline"] = " // Backup of InnerWindow->WorkRect at the end of BeginTable()"
...
defs["structs"]["ImGuiTableTempData"][6]["comment"]["sameline"] = " // Backup of InnerWindow->ParentWorkRect at the end of BeginTable()"
...
defs["structs"]["ImGuiTableTempData"][7]["comment"]["sameline"] = " // Backup of InnerWindow->DC.PrevLineSize at the end of BeginTable()"
...
defs["structs"]["ImGuiTableTempData"][8]["comment"]["sameline"] = " // Backup of InnerWindow->DC.CurrLineSize at the end of BeginTable()"
...
defs["structs"]["ImGuiTableTempData"][9]["comment"]["sameline"] = " // Backup of InnerWindow->DC.CursorMaxPos at the end of BeginTable()"
...
defs["structs"]["ImGuiTableTempData"][10]["comment"]["sameline"] = " // Backup of OuterWindow->DC.ColumnsOffset at the end of BeginTable()"
...
defs["structs"]["ImGuiTableTempData"][11]["comment"]["sameline"] = " // Backup of OuterWindow->DC.ItemWidth at the end of BeginTable()"
...
defs["structs"]["ImGuiTableTempData"][12]["comment"]["sameline"] = "//Backup of OuterWindow->DC.ItemWidthStack.Size at the end of BeginTable()"
...
defs["structs"]["ImGuiViewport"][14]["comment"]["sameline"] = " // Platform window requested move (e.g. window was moved by the OS / host window manager, authoritative position will be OS window position)"
...
defs["structs"]["ImGuiViewport"][15]["comment"]["sameline"] = " // Platform window requested resize (e.g. window was resized by the OS / host window manager, authoritative size will be OS window size)"
...
defs["structs"]["ImGuiViewport"][16]["comment"]["sameline"] = " // Platform window requested closure (e.g. window was moved by the OS / host window manager, e.g. pressing ALT-F4)"
...
defs["structs"]["ImGuiViewportP"][3]["comment"]["sameline"] = " // Last frame number this viewport was activated by a window"
...
defs["structs"]["ImGuiViewportP"][4]["comment"]["sameline"] = "// Last stamp number from when a window hosted by this viewport was made front-most (by comparing this value between two viewport we have an implicit viewport z-order"
...
defs["structs"]["ImGuiViewportP"][7]["comment"]["sameline"] = " // Window opacity (when dragging dockable windows/viewports we make them transparent)"
...
defs["structs"]["ImGuiViewportP"][11]["comment"]["sameline"] = " // Set when the viewport is owned by a window (and ImGuiViewportFlags_CanHostOtherWindows is NOT set)"
...
defs["structs"]["ImGuiWindow"][1]["comment"]["sameline"] = " // Window name, owned by the window."
...
defs["structs"]["ImGuiWindow"][2]["comment"]["sameline"] = " // == ImHashStr(Name)"
...
defs["structs"]["ImGuiWindow"][3]["comment"]["sameline"] = " // See enum ImGuiWindowFlags_"
...
defs["structs"]["ImGuiWindow"][5]["comment"]["sameline"] = " // Advanced users only. Set with SetNextWindowClass()"
...
defs["structs"]["ImGuiWindow"][6]["comment"]["sameline"] = " // Always set in Begin(). Inactive windows may have a NULL value here if their viewport was discarded."
...
defs["structs"]["ImGuiWindow"][7]["comment"]["sameline"] = " // We backup the viewport id (since the viewport may disappear or never be created if the window is inactive)"
...
defs["structs"]["ImGuiWindow"][8]["comment"]["sameline"] = " // We backup the viewport position (since the viewport may disappear or never be created if the window is inactive)"
...
defs["structs"]["ImGuiWindow"][9]["comment"]["sameline"] = " // Reset to -1 every frame (index is guaranteed to be valid between NewFrame..EndFrame), only used in the Appearing frame of a tooltip/popup to enforce clamping to a given monitor"
...
defs["structs"]["ImGuiWindow"][10]["comment"]["sameline"] = " // Position (always rounded-up to nearest pixel)"
...
defs["structs"]["ImGuiWindow"][11]["comment"]["sameline"] = " // Current size (==SizeFull or collapsed title bar size)"
...
defs["structs"]["ImGuiWindow"][12]["comment"]["sameline"] = " // Size when non collapsed"
...
defs["structs"]["ImGuiWindow"][13]["comment"]["sameline"] = " // Size of contents/scrollable client area (calculated from the extents reach of the cursor) from previous frame. Does not include window decoration or window padding."
...
defs["structs"]["ImGuiWindow"][15]["comment"]["sameline"] = " // Size of contents/scrollable client area explicitly request by the user via SetNextWindowContentSize()."
...
defs["structs"]["ImGuiWindow"][16]["comment"]["sameline"] = " // Window padding at the time of Begin()."
...
defs["structs"]["ImGuiWindow"][17]["comment"]["sameline"] = " // Window rounding at the time of Begin(). May be clamped lower to avoid rendering artifacts with title bar, menu bar etc."
...
defs["structs"]["ImGuiWindow"][18]["comment"]["sameline"] = " // Window border size at the time of Begin()."
...
defs["structs"]["ImGuiWindow"][19]["comment"]["sameline"] = " // Size of buffer storing Name. May be larger than strlen(Name)!"
...
defs["structs"]["ImGuiWindow"][20]["comment"]["sameline"] = " // == window->GetID(\"#MOVE\")"
...
defs["structs"]["ImGuiWindow"][21]["comment"]["sameline"] = " // == window->GetID(\"#TAB\")"
...
defs["structs"]["ImGuiWindow"][22]["comment"]["sameline"] = " // ID of corresponding item in parent window (for navigation to return from child window to parent window)"
...
defs["structs"]["ImGuiWindow"][25]["comment"]["sameline"] = " // target scroll position. stored as cursor position with scrolling canceled out, so the highest point is always 0.0f. (FLT_MAX for no change)"
...
defs["structs"]["ImGuiWindow"][26]["comment"]["sameline"] = " // 0.0f = scroll so that target position is at top, 0.5f = scroll so that target position is centered"
...
defs["structs"]["ImGuiWindow"][27]["comment"]["sameline"] = " // 0.0f = no snapping, >0.0f snapping threshold"
...
defs["structs"]["ImGuiWindow"][28]["comment"]["sameline"] = " // Size taken by each scrollbars on their smaller axis. Pay attention! ScrollbarSizes.x == width of the vertical scrollbar, ScrollbarSizes.y = height of the horizontal scrollbar."
...
defs["structs"]["ImGuiWindow"][29]["comment"]["sameline"] = " // Are scrollbars visible?"
...
defs["structs"]["ImGuiWindow"][32]["comment"]["sameline"] = " // Set to true on Begin(), unless Collapsed"
...
defs["structs"]["ImGuiWindow"][34]["comment"]["sameline"] = " // Set to true when any widget access the current window"
...
defs["structs"]["ImGuiWindow"][35]["comment"]["sameline"] = " // Set when collapsing window to become only title-bar"
...
defs["structs"]["ImGuiWindow"][37]["comment"]["sameline"] = " // Set when items can safely be all clipped (e.g. window not visible or collapsed)"
...
defs["structs"]["ImGuiWindow"][38]["comment"]["sameline"] = " // Set during the frame where the window is appearing (or re-appearing)"
...
defs["structs"]["ImGuiWindow"][39]["comment"]["sameline"] = " // Do not display (== HiddenFrames*** > 0)"
...
defs["structs"]["ImGuiWindow"][40]["comment"]["sameline"] = " // Set on the \"Debug##Default\" window."
...
defs["structs"]["ImGuiWindow"][41]["comment"]["sameline"] = " // Set when passed _ChildWindow, left to false by BeginDocked()"
...
defs["structs"]["ImGuiWindow"][42]["comment"]["sameline"] = " // Set when the window has a close button (p_open != NULL)"
...
defs["structs"]["ImGuiWindow"][43]["comment"]["sameline"] = " // Current border being held for resize (-1: none, otherwise 0-3)"
...
defs["structs"]["ImGuiWindow"][44]["comment"]["sameline"] = " // Number of Begin() during the current frame (generally 0 or 1, 1+ if appending via multiple Begin/End pairs)"
...
defs["structs"]["ImGuiWindow"][45]["comment"]["sameline"] = " // Begin() order within immediate parent window, if we are a child window. Otherwise 0."
...
defs["structs"]["ImGuiWindow"][46]["comment"]["sameline"] = " // Begin() order within entire imgui context. This is mostly used for debugging submission order related issues."
...
defs["structs"]["ImGuiWindow"][47]["comment"]["sameline"] = " // Order within WindowsFocusOrder[], altered when windows are focused."
...
defs["structs"]["ImGuiWindow"][48]["comment"]["sameline"] = " // ID in the popup stack when this window is used as a popup/menu (because we use generic Name/ID for recycling)"
...
defs["structs"]["ImGuiWindow"][54]["comment"]["sameline"] = " // Hide the window for N frames"
...
defs["structs"]["ImGuiWindow"][55]["comment"]["sameline"] = " // Hide the window for N frames while allowing items to be submitted so we can measure their size"
...
defs["structs"]["ImGuiWindow"][56]["comment"]["sameline"] = " // Hide the window until frame N at Render() time only"
...
defs["structs"]["ImGuiWindow"][57]["comment"]["sameline"] = " // Disable window interactions for N frames"
...
defs["structs"]["ImGuiWindow"][58]["comment"]["sameline"] = " // store acceptable condition flags for SetNextWindowPos() use."
...
defs["structs"]["ImGuiWindow"][59]["comment"]["sameline"] = " // store acceptable condition flags for SetNextWindowSize() use."
...
defs["structs"]["ImGuiWindow"][60]["comment"]["sameline"] = " // store acceptable condition flags for SetNextWindowCollapsed() use."
...
defs["structs"]["ImGuiWindow"][61]["comment"]["sameline"] = " // store acceptable condition flags for SetNextWindowDock() use."
...
defs["structs"]["ImGuiWindow"][62]["comment"]["sameline"] = " // store window position when using a non-zero Pivot (position set needs to be processed when we know the window size)"
...
defs["structs"]["ImGuiWindow"][63]["comment"]["sameline"] = " // store window pivot for positioning. ImVec2(0, 0) when positioning from top-left corner; ImVec2(0.5f, 0.5f) for centering; ImVec2(1, 1) for bottom right."
...
defs["structs"]["ImGuiWindow"][64]["comment"]["sameline"] = " // ID stack. ID are hashes seeded with the value at the top of the stack. (In theory this should be in the TempData structure)"
...
defs["structs"]["ImGuiWindow"][65]["comment"]["sameline"] = " // Temporary per-window data, reset at the beginning of the frame. This used to be called ImGuiDrawContext, hence the \"DC\" variable name."
...
defs["structs"]["ImGuiWindow"][66]["comment"]["above"] = "    // The best way to understand what those rectangles are is to use the 'Metrics->Tools->Show Windows Rectangles' viewer.\
    // The main 'OuterRect', omitted as a field, is window->Rect()."
defs["structs"]["ImGuiWindow"][66]["comment"]["sameline"] = " // == Window->Rect() just after setup in Begin(). == window->Rect() for root window."
...
defs["structs"]["ImGuiWindow"][67]["comment"]["sameline"] = " // Inner rectangle (omit title bar, menu bar, scroll bar)"
...
defs["structs"]["ImGuiWindow"][68]["comment"]["sameline"] = " // == InnerRect shrunk by WindowPadding*0.5f on each side, clipped within viewport or parent clip rect."
...
defs["structs"]["ImGuiWindow"][69]["comment"]["sameline"] = " // Initially covers the whole scrolling region. Reduced by containers e.g columns/tables when active. Shrunk by WindowPadding*1.0f on each side. This is meant to replace ContentRegionRect over time (from 1.71+ onward)."
...
defs["structs"]["ImGuiWindow"][70]["comment"]["sameline"] = " // Backup of WorkRect before entering a container such as columns/tables. Used by e.g. SpanAllColumns functions to easily access. Stacked containers are responsible for maintaining this. // FIXME-WORKRECT: Could be a stack?"
...
defs["structs"]["ImGuiWindow"][71]["comment"]["sameline"] = " // Current clipping/scissoring rectangle, evolve as we are using PushClipRect(), etc. == DrawList->clip_rect_stack.back()."
...
defs["structs"]["ImGuiWindow"][72]["comment"]["sameline"] = " // FIXME: This is currently confusing/misleading. It is essentially WorkRect but not handling of scrolling. We currently rely on it as right/bottom aligned sizing operation need some size to rely on."
...
defs["structs"]["ImGuiWindow"][73]["comment"]["sameline"] = " // Define an optional rectangular hole where mouse will pass-through the window."
...
defs["structs"]["ImGuiWindow"][75]["comment"]["sameline"] = " // Last frame number the window was Active."
...
defs["structs"]["ImGuiWindow"][76]["comment"]["sameline"] = " // Last frame number the window was made Focused."
...
defs["structs"]["ImGuiWindow"][77]["comment"]["sameline"] = " // Last timestamp the window was Active (using float as we don't need high precision there)"
...
defs["structs"]["ImGuiWindow"][81]["comment"]["sameline"] = " // User scale multiplier per-window, via SetWindowFontScale()"
...
defs["structs"]["ImGuiWindow"][83]["comment"]["sameline"] = " // Offset into SettingsWindows[] (offsets are always valid as we only grow the array from the back)"
...
defs["structs"]["ImGuiWindow"][84]["comment"]["sameline"] = " // == &DrawListInst (for backward compatibility reason with code using imgui_internal.h we keep this a pointer)"
...
defs["structs"]["ImGuiWindow"][86]["comment"]["sameline"] = " // If we are a child _or_ popup _or_ docked window, this is pointing to our parent. Otherwise NULL."
...
defs["structs"]["ImGuiWindow"][88]["comment"]["sameline"] = " // Point to ourself or first ancestor that is not a child window. Doesn't cross through popups/dock nodes."
...
defs["structs"]["ImGuiWindow"][89]["comment"]["sameline"] = " // Point to ourself or first ancestor that is not a child window. Cross through popups parent<>child."
...
defs["structs"]["ImGuiWindow"][90]["comment"]["sameline"] = " // Point to ourself or first ancestor that is not a child window. Cross through dock nodes."
...
defs["structs"]["ImGuiWindow"][91]["comment"]["sameline"] = " // Point to ourself or first ancestor which will display TitleBgActive color when this window is active."
...
defs["structs"]["ImGuiWindow"][92]["comment"]["sameline"] = " // Point to ourself or first ancestor which doesn't have the NavFlattened flag."
...
defs["structs"]["ImGuiWindow"][93]["comment"]["sameline"] = " // When going to the menu bar, we remember the child window we came from. (This could probably be made implicit if we kept g.Windows sorted by last focused including child window.)"
...
defs["structs"]["ImGuiWindow"][94]["comment"]["sameline"] = " // Last known NavId for this window, per layer (0/1)"
...
defs["structs"]["ImGuiWindow"][95]["comment"]["sameline"] = " // Reference rectangle, in window relative space"
...
defs["structs"]["ImGuiWindow"][96]["comment"]["sameline"] = " // Backup of last idx/vtx count, so when waking up the window we can preallocate and avoid iterative alloc/copy"
...
defs["structs"]["ImGuiWindow"][98]["comment"]["sameline"] = " // Set when window extraneous data have been garbage collected"
...
defs["structs"]["ImGuiWindow"][99]["comment"]["above"] = "    // Docking"
defs["structs"]["ImGuiWindow"][99]["comment"]["sameline"] = " // When docking artifacts are actually visible. When this is set, DockNode is guaranteed to be != NULL. ~~ (DockNode != NULL) && (DockNode->Windows.Size > 1)."
...
defs["structs"]["ImGuiWindow"][101]["comment"]["sameline"] = " // Is our window visible this frame? ~~ is the corresponding tab selected?"
...
defs["structs"]["ImGuiWindow"][103]["comment"]["sameline"] = " // Order of the last time the window was visible within its DockNode. This is used to reorder windows that are reappearing on the same frame. Same value between windows that were active and windows that were none are possible."
...
defs["structs"]["ImGuiWindow"][105]["comment"]["sameline"] = " // Which node are we docked into. Important: Prefer testing DockIsActive in many cases as this will still be set when the dock node is hidden."
...
defs["structs"]["ImGuiWindow"][106]["comment"]["sameline"] = " // Which node are we owning (for parent windows)"
...
defs["structs"]["ImGuiWindow"][107]["comment"]["sameline"] = " // Backup of last valid DockNode->ID, so single window remember their dock node id even when they are not bound any more"
...
defs["structs"]["ImGuiWindowClass"][3]["comment"]["sameline"] = " // Viewport flags to set when a window of this class owns a viewport. This allows you to enforce OS decoration or task bar icon, override the defaults on a per-window basis."
...
defs["structs"]["ImGuiWindowClass"][4]["comment"]["sameline"] = " // Viewport flags to clear when a window of this class owns a viewport. This allows you to enforce OS decoration or task bar icon, override the defaults on a per-window basis."
...
defs["structs"]["ImGuiWindowClass"][5]["comment"]["sameline"] = " // [EXPERIMENTAL] TabItem flags to set when a window of this class gets submitted into a dock node tab bar. May use with ImGuiTabItemFlags_Leading or ImGuiTabItemFlags_Trailing."
...
defs["structs"]["ImGuiWindowClass"][6]["comment"]["sameline"] = " // [EXPERIMENTAL] Dock node flags to set when a window of this class is hosted by a dock node (it doesn't have to be selected!)"
...
defs["structs"]["ImGuiWindowClass"][7]["comment"]["sameline"] = " // Set to true to enforce single floating windows of this class always having their own docking node (equivalent of setting the global io.ConfigDockingAlwaysTabBar)"
...
defs["structs"]["ImGuiWindowClass"][8]["comment"]["sameline"] = " // Set to true to allow windows of this class to be docked/merged with an unclassed window. // FIXME-DOCK: Move to DockNodeFlags override?"
...
defs["structs"]["ImGuiWindowSettings"][10]["comment"]["sameline"] = " // Set when loaded from .ini data (to enable merging/loading .ini data into an already running context)"
...
defs["structs"]["ImGuiWindowTempData"][11]["comment"]["sameline"] = " // Indentation / start position from left of window (increased by TreePush/TreePop, etc.)"
...
defs["structs"]["ImGuiWindowTempData"][12]["comment"]["sameline"] = " // Offset to the current column (if ColumnsCurrent > 0). FIXME: This and the above should be a stack to allow use cases like Tree->Column->Tree. Need revamp columns API."
...
defs["structs"]["ImGuiWindowTempData"][14]["comment"]["sameline"] = "// Record the loss of precision of CursorStartPos due to really large scrolling amount. This is used by clipper to compensentate and fix the most common use case of large scroll area."
...
defs["structs"]["ImGuiWindowTempData"][15]["comment"]["above"] = "    // Keyboard/Gamepad navigation"
defs["structs"]["ImGuiWindowTempData"][15]["comment"]["sameline"] = " // Current layer, 0..31 (we currently only use 0..1)"
...
defs["structs"]["ImGuiWindowTempData"][16]["comment"]["sameline"] = " // Which layers have been written to (result from previous frame)"
...
defs["structs"]["ImGuiWindowTempData"][17]["comment"]["sameline"] = "// Which layers have been written to (accumulator for current frame)"
...
defs["structs"]["ImGuiWindowTempData"][18]["comment"]["sameline"] = " // Current focus scope ID while appending"
...
defs["structs"]["ImGuiWindowTempData"][20]["comment"]["sameline"] = " // Set when scrolling can be used (ScrollMax > 0.0f)"
...
defs["structs"]["ImGuiWindowTempData"][21]["comment"]["above"] = "    // Miscellaneous"
defs["structs"]["ImGuiWindowTempData"][21]["comment"]["sameline"] = " // FIXME: Remove this"
...
defs["structs"]["ImGuiWindowTempData"][22]["comment"]["sameline"] = " // MenuBarOffset.x is sort of equivalent of a per-layer CursorPos.x, saved/restored as we switch to the menu bar. The only situation when MenuBarOffset.y is > 0 if when (SafeAreaPadding.y > FramePadding.y), often used on TVs."
...
defs["structs"]["ImGuiWindowTempData"][23]["comment"]["sameline"] = " // Simplified columns storage for menu items measurement"
...
defs["structs"]["ImGuiWindowTempData"][24]["comment"]["sameline"] = " // Current tree depth."
...
defs["structs"]["ImGuiWindowTempData"][25]["comment"]["sameline"] = " // Store a copy of !g.NavIdIsAlive for TreeDepth 0..31.. Could be turned into a ImU64 if necessary."
...
defs["structs"]["ImGuiWindowTempData"][27]["comment"]["sameline"] = " // Current persistent per-window storage (store e.g. tree node open/close state)"
...
defs["structs"]["ImGuiWindowTempData"][28]["comment"]["sameline"] = " // Current columns set"
...
defs["structs"]["ImGuiWindowTempData"][29]["comment"]["sameline"] = " // Current table index (into g.Tables)"
...
defs["structs"]["ImGuiWindowTempData"][31]["comment"]["sameline"] = " // Layout type of parent window at the time of Begin()"
...
defs["structs"]["ImGuiWindowTempData"][32]["comment"]["above"] = "    // Local parameters stacks\
    // We store the current settings outside of the vectors to increase memory locality (reduce cache misses). The vectors are rarely modified. Also it allows us to not heap allocate for short-lived windows which are not using those settings."
defs["structs"]["ImGuiWindowTempData"][32]["comment"]["sameline"] = " // Current item width (>0.0: width in pixels, <0.0: align xx pixels to the right of window)."
...
defs["structs"]["ImGuiWindowTempData"][33]["comment"]["sameline"] = " // Current text wrap pos."
...
defs["structs"]["ImGuiWindowTempData"][34]["comment"]["sameline"] = " // Store item widths to restore (attention: .back() is not == ItemWidth)"
...
defs["structs"]["ImGuiWindowTempData"][35]["comment"]["sameline"] = " // Store text wrap pos to restore (attention: .back() is not == TextWrapPos)"
